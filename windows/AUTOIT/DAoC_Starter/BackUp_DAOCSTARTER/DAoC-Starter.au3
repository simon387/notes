...
Global Const $title = "DAoC Starter 1.110 rev:5"
...
$Combo_general = GUICtrlCreateCombo("on|off", 145, 36, 145, 25, BitOR($CBS_DROPDOWNLIST, $CBS_AUTOHSCROLL))
...
GUICtrlSetOnEvent($MenuItem_patch, 'patch')
...
GUICtrlSetOnEvent($MenuItem_manage, 'accountManager')
...
GUICtrlSetOnEvent($MenuItem_change, 'setUserDD')
GUICtrlSetOnEvent($MenuItem_f5, 'updateGui')
...
Func openAdvancedOptions()
	...
	GUISwitch($Form3)
	GUISetOnEvent($GUI_EVENT_CLOSE, 'chiudiAO')
	GUICtrlSetOnEvent($Combo_general, 'setGeneralAudio')
	GUICtrlSetOnEvent($Combo_ambient_m, 'setComboSounds')
	GUICtrlSetOnEvent($Combo_ambient_s, 'setComboSounds')
	GUICtrlSetOnEvent($Combo_music, 'setComboSounds')
	GUICtrlSetOnEvent($Combo_sound, 'setComboSounds')
	GUICtrlSetOnEvent($Combo_sleep, 'setCombo')
	GUICtrlSetOnEvent($Combo_shadow, 'setCombo')
	GUICtrlSetOnEvent($Combo_dynamic, 'setCombo')
	GUICtrlSetOnEvent($Combo_water, 'setCombo')
	GUICtrlSetOnEvent($Combo_reflection_q, 'setCombo')
	GUICtrlSetOnEvent($Combo_reflection_u, 'setCombo')
	GUICtrlSetOnEvent($Combo_atlantis_t, 'atlantis_internal')
	GUICtrlSetOnEvent($Combo_atlantis_te, 'atlantis_internal')
	GUICtrlSetOnEvent($Combo_characters, 'setCharVer')
EndFunc
...
Func accountManager()
	...
	GUISwitch($Form2)
	GUICtrlSetOnEvent($Button_back, 'chiudiAccountManager')
	GUISetOnEvent($GUI_EVENT_CLOSE, 'chiudiAccountManager')
	GUICtrlSetOnEvent($Button_add, 'addAccount')
	GUICtrlSetOnEvent($Button_edit, 'editAccount')
	GUICtrlSetOnEvent($Button_remove, 'deleteAccount')
	GUICtrlSetOnEvent($Button_show, 'showPassword')
EndFunc
...
Func showPassword()
	...
	If @error <> 1 Then
		...
		GUISwitch($Formi)
		GUISetOnEvent($GUI_EVENT_CLOSE, 'closeShowPassword')
	EndIf
EndFunc
...
Func deleteEntry()
	...
	Else
		...
		Next
		$array = arrayCompresser($array, $temp)
		For $i = 0 To 9
...
Func arrayCompresser($array, $element)
	Dim $tempArray[UBound($array) + 1]
	For $i = 0 To UBound($array) - 1
		$tempArray[$i] = $array[$i]
	Next
	$tempArray[UBound($array) - 1] = ""
	For $i = 0 To UBound($array) - 1
		If $array[$i] <> $element Then
			$tempArray[$i] = $array[$i]
		Else
			For $c = $i To UBound($array) - 1
...
Func killClient()
	...
	GUISwitch($Form4)
	GUISetOnEvent($GUI_EVENT_CLOSE, 'quitForm4')
	GUICtrlSetOnEvent($Button2, 'quitForm4')
	GUICtrlSetOnEvent($List1, 'flash')
	GUICtrlSetOnEvent($Button1, 'killSelected')
EndFunc
...
Func flash()
	If GUICtrlRead($List1) <> "" Then WinFlash($mega[GUICtrlRead($List1)][1], Default, 2, 50)
EndFunc
...
Func irc()
	ShellExecute("http://www.sorcery.net/index.php?option=com_wrapper&view=wrapper&Itemid=69")
	GUICtrlSetData($Label_, "Starting IRC website")
EndFunc
...
Func info()
	If @GUI_CtrlId == $MenuItem_about2 Then
		MsgBox(64, "Informations & thanks to", "Created by simon387@hotmail.it aka Rayvaughan." & @CR & @CR & "Testers and ideas: C@stro, hawk'~, Madcastah, Vindicare." & @CR & @CR & "Handle.exe Copyright (C) 1997-2012 Mark Russinovich." & @CR & "DOLLoader - DOL Project." & @CR & "Ventrilo© Flagship Industries, Inc. All right reserved." & @CR & "TeamSpeak© TeamSpeak Systems GmbH. All rights reserved." & @CR & "Dark Age of Camelot, Copyright (C) 2001-12 Mythic Entertainment, Inc.", "", $Form1)
	Else
...
Func mutexKiller()
	Local $file, $hex
	If IsAdmin() <> 1 Then GUICtrlSetData($Label_, "No admin priv. !")
	FileInstall("C:\ZZZ\AUTOIT\DAoC_Starter\handle.exe", $path & "\handle.exe");FileInstall("C:\ZZZ\AUTOIT\DAoC_Starter\Eula.txt", $path & "\Eula.txt")
